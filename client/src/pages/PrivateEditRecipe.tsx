  ...
  const { loading } = useQuery(GET_PRIVATE_RECIPES, {
    ...
    onCompleted: (data) => {
      if (data.me.privateRecipes.length) {
        ...
        privateRecipe.estimatedTime &&
          setEstimatedTime(privateRecipe.estimatedTime);
        setToggle(privateRecipe.isPublic);
        privateRecipe.ingredients.forEach((ingredient: Ingredient) => {
          append(ingredient);
        });
      }
    },
...
export default PrivateEditRecipe;
