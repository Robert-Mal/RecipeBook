...
import { GET_ME, GET_PRIVATE_RECIPES } from "../GraphQL/Queries";
import { GET_MY_RECIPES_AND_EATING_PLANS } from "../GraphQL/ShoppingLists/Queries";
...
const NewRecipe = () => {
  ...
  const [createRecipe] = useMutation(CREATE_RECIPE, {
    ...
    refetchQueries: [
      ...
      { query: GET_MY_RECIPES_AND_EATING_PLANS },
    ],
    onCompleted: (data) => {
      toast.success("Recipe created");
      if (toggle) {
        navigate("/dashboard/recipe/" + data.createRecipe._id, {
          replace: true,
        });
      } else {
        navigate("/dashboard/recipe/private/" + data.createRecipe._id, {
          replace: true,
        });
      }
    },
  ...
  const onSubmit = async (recipe: any) => {
    ...
    await createRecipe({ variables: { createRecipeInput: recipe } });
  };
  ...
  return (
    <div className="flex h-full w-full justify-center py-5 overflow-auto">
      ...
      >
        ...
        <p className="text-4xl font-bold mb-4">Create Recipe</p>
        <div className="mb-4">
          ...
          />
          {errors.name ? (
            <p data-cy="error-name" className="text-red-500 text-sm ml-7">
          ...
          <li key={field.id} className="flex items-start">
            <div className="w-1/3 m-5">
              <input
                type="text"
                placeholder="Name"
                data-cy={`ingredients.${index}.name`}
            ...
            <div className="w-1/3 m-5">
              <input
                ...
                data-cy={`ingredients.${index}.quantity`}
                className="shadow appearance-none border rounded py-2 px-3 leading-tight focus:border-gray-500 focus:outline-none focus:shadow-outline w-full h-10"
            ...
            <div className="w-1/3 m-5">
              <input
                ...
                data-cy={`ingredients.${index}.unit`}
                className="shadow appearance-none border rounded py-2 px-3 leading-tight focus:border-gray-500 focus:outline-none focus:shadow-outline w-full h-10"
        ...
        <div
          ...
          className="flex mb-4 items-center cursor-pointer"
        >
          <p data-cy="show-preview" className="block text-gray-700 font-bold">
            Show preview
          </p>
        </div>
